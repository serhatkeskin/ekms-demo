...
function Sidenav({ color, brand, brandName, routes, ...rest }: any) {
  ...
  // Get auth context to check user staff status
  const [auth] = useAuth();
  ...
  // Filter routes based on user permissions before rendering
  const filteredRoutes = routes.filter(route => {
    // Only show items in sidebar that should be in the sidebar
  ...
  // Function to render a single route item
  const renderRoute = (item, level = 0) => {
    const { type, name, icon, title, noCollapse, key, href, route, collapse } = item;
    const paddingLeft = level > 0 ? { paddingLeft: level * 2 + 'rem' } : {};

    if (type === "collapse") {
      // If it has nested items and isn't supposed to collapse
      if (collapse && collapse.length > 0) {
        return (
          <MDBox key={key}>
            ...
            />
            <List sx={{ pl: level + 1 }}>
              {collapse.map((collapseItem, index) => renderRoute(collapseItem, level + 1))}
            </List>
          </MDBox>
        );
      }
  ...
  const renderRoutes = filteredRoutes.map(item => renderRoute(item));
